...
class Scanner
{
	...
	public function check_system_requirements()
	{
		...
		if (in_array('exec', $disabled, true)) {
			...
			$status['messages'][]  = __('exec() is disabled in php.ini', 'phpcompatibility-checker');
		}
		...
		if ($status['execEnabled'] && $status['phpBinaryExists'] && $status['phpcsExists']) {
			...
			if (! $version_result['success']) {
				$status['messages'][] = __('Unable to run phpcs --version with detected PHP binary.', 'phpcompatibility-checker');
			}
		}
